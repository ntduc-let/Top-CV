...
class MenuProfessionAdapter(
    context: Context,
    private var list: List<Profession>
) : ArrayAdapter<Profession>(context, 0, list), Filterable {
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
        ...
        if (gender != null) {
            binding.item.text = gender.professionDB.name
        }
        return binding.root
    }
        ...
        return object : Filter(){
            ...
            override fun publishResults(constraint: CharSequence?, results: FilterResults?) {
                if (results != null && results.count > 0) {
                    list = results.values as List<Profession>
            ...
            override fun convertResultToString(resultValue: Any?): CharSequence {
                return (resultValue as Profession).professionDB.name.toString()
            }
        }
    }

    private fun findProfession(gender: String): ArrayList<Profession> {
        val result = arrayListOf<Profession>()
        try {
            list.forEach {
                if (it.professionDB.name!!.contains(gender, true)) {
                    result.add(it)
                }
            }
        } catch (e: Exception) {
