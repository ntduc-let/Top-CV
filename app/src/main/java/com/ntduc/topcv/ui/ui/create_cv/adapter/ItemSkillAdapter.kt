...
class ItemSkillAdapter(
    val context: Context,
    val mode: Int,
    var list: ArrayList<Skill> = arrayListOf()
) :
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        if (holder is ShowSkillViewHolder) {
            with(holder) {
    ...
    fun updateData(newList: List<Skill>) {
        list = arrayListOf()
        newList.forEach {
            list.add(it)
        }
    ...
    inner class ShowSkillViewHolder(binding: ItemSkillShowBinding) :
    ...
    private var onClickItemListener: ((Skill) -> Unit)? = null
    fun setOnClickItemListener(listener: (Skill) -> Unit) {
